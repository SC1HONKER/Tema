...
banner() {
	cat <<- EOF
		...
		${GREEN}[${WHITE}-${RED}]${CYAN} Created by BLACKTERMUX Programmer$ (Tanmay.Tiwari){WHITE}
	EOF
}
...
## Dependencies
dependencies() {
	echo -e "\n${GREEN}[${WHITE}+${RED}]${CYAN} Installing required packages..."
...
msg_exit() {
	{ clear; banner; echo; }
	echo -e "${GREENBG}${BLACK} Thank you for using PHISH.${RESETBG}\n"
	{ reset_color; exit 0; }
}
...
about() {
	{ clear; banner; echo; }
	cat <<- EOF
		${GREEN}Author   ${RED}:  ${RED}BLACKTERMUX ${RED}[ ${ORANGE}HTR-TECH ${RED}]
		...
		${RED}Author will not be responsible for any misuse of this BLACKTERMUX ${RED}!${WHITE}
	...
	case $REPLY in 
		99)
			msg_exit;;
		0 | 00)
			echo -ne "\n${GREEN}[${WHITE}+${RED}]${CYAN} Returning to main menu..."
			{ sleep 1; main_menu; };;
		*)
